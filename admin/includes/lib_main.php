...
{
    if(isset($GLOBALS['_LANG']['log_action'][$action]) && isset($GLOBALS['_LANG']['log_action'][$content])){
        $log_info = $GLOBALS['_LANG']['log_action'][$action] . $GLOBALS['_LANG']['log_action'][$content];
    } else {
        $log_info = $content;
    }
    if(!empty($sn)){
        $log_info .= !empty($log_info) ? ': '. addslashes($sn) : $sn;
    }

    $GLOBALS['dbi']->insert('admin_log', array(
        'log_time' => gmtime(),
...

{
    ...
    elseif ($cache->exists('page_size') && intval($cache->get('page_size')) > 0)

    {

        $filter['page_size'] = intval($cache->get('page_size'));
...

{
    ...
    $cache = cache::init();
    $cache->setex('lastfilterfile', 600, sprintf('%X', crc32($filterfile)));
    $cache->setex('lastfilter', 600, urlencode(serialize($filter)));
    $cache->setex('lastfiltersql', 600, base64_encode($sql));
}
...

{
    ...
    $cache = cache::init();
    if (isset($_GET['uselastfilter']) && $cache->exists('lastfilterfile')

        && $cache->get('lastfilterfile') == sprintf('%X', crc32($filterfile)))

    {

        return array(

            'filter' => unserialize(urldecode($cache->get('lastfilter'))),

            'sql'    => base64_decode($cache->get('lastfiltersql'))
